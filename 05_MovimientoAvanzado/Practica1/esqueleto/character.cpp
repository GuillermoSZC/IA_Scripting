...
Character::Character() : mLinearVelocity(0.0f, 0.0f), mAngularVelocity(0.0f)
{
    ...
    steering.push_back(std::make_unique<ArriveSteering>(this));
    steering.push_back(std::make_unique<AlignToMovementSteering>(this));
    steering.push_back(std::make_unique<PathFollowing>(this));
}
...
void Character::OnUpdate(float step)
{
    ...
    SteeringOperations steeringOp;
    for (auto& iter : steering)
    {
        if (iter)
        {
            iter->OnUpdate(step);
            steeringOp += iter->GetSteering();
        }
    }

    mLinearVelocity += steeringOp.linear * step;
    mAngularVelocity += steeringOp.angular * step;
