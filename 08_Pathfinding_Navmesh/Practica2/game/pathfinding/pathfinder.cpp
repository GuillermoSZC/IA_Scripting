...
{
    if (path->EmptyPath() && !openNodes.empty())
    {
        ...
        {
            Polygon* nextPolygon = link.polygon;
            Node* repeatedNode = GetVectorNode(closeNodes, nextPolygon);

            if (!repeatedNode)
            {
                repeatedNode = GetVectorNode(openNodes, nextPolygon);
                const float costG = currentNode->gCost + 1.f;
                ...
                {
                    const USVec2D pos = currentNode->polygon->GetMiddlePoint(link);
                    auto node = new Node(currentNode, pos, nextPolygon,
                        costG, HeuristicCalculation(pos));
